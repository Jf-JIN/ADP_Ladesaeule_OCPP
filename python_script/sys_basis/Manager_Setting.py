...
class SettingManager():
    ...
    def __init__(self, exe_folder_path: str, default_setting_dict: dict = {}, default_name: str = '') -> None:
    ...
    def default_setting_dict(self):
        return self.__default_setting_dict
    ...
    def open_file_to_json(self, file_path: str) -> None | dict:
        ...
        """
        if not isinstance(file_path, str) or not file_path or file_path == '':
            print('file_path 必须为 str 类型, 且不能为空')
            return
        ...
        def __scan(current_key: str, current_value, default_value) -> None:
            ...
            if isinstance(default_value, dict):
                ...
                if not isinstance(current_value, expected_type):
                    ...
                    flag_write = True
        __scan('', content, self.__default_setting_dict)
        if flag_write:
    ...
    def __check_redundancy(self, content: dict) -> dict:
        ...
        def __scan(parent: dict, default: dict) -> None:
            ...
            for key in keys_to_delete:
                del parent[key]
        __scan(compare_dict, self.__default_setting_dict)
        if flag_write:
    ...
    def __check_lost(self, content: dict) -> dict:
        ...
        __scan(self.__default_setting_dict, content, temp)
        if flag_write:
            self.write_file_to_json(temp, self.__setting_path)
        return temp
    ...
    def __rebuild_setting(self) -> dict:
        ...
        """
        temp = {}
        self.__scan_to_build(self.__default_setting_dict, temp)
        self.write_file_to_json(temp, self.__setting_path)
        return temp
