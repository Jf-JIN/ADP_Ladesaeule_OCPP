...
const btn_synchronize_time = document.getElementById('synchronize_time');
btn_synchronize_time.addEventListener('click', () => {
    let now_time = new Date();
    let year_t = String(now_time.getFullYear()).padStart(2, '0');
    let month_t = String(now_time.getMonth() + 1).padStart(2, '0');
    let day_t = String(now_time.getDate()).padStart(2, '0');
    let hour_t = String(now_time.getHours()).padStart(2, '0');
    let minute_t = String(now_time.getMinutes()).padStart(2, '0');
    let second_t = String(now_time.getSeconds()).padStart(2, '0');
    let time_str = `${year_t}-${month_t}-${day_t} ${hour_t}:${minute_t}:${second_t}`
    socket.emit('input_data', { "time_synchronize": { "target_time": time_str } });
})
