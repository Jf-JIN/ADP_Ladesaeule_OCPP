...
class GenSetDisplayMessageResponse(Base_OCPP_Struct_V2_0_1):

    @staticmethod
    def generate(
        status: str | DisplayMessageStatusType,
        status_info: dict | None = None,
        custom_data: dict | None = None
    ) -> call_result.SetDisplayMessage:
        """
        Generate SetDisplayMessageResponse

        - Args: 
            - status(str): 
                - This indicates whether the Charging Station is able to display the message. 
                - Enum: `Accepted`, `NotSupportedMessageFormat`, `Rejected`, `NotSupportedPriority`, `NotSupportedState`, `UnknownTransaction`
                - Or use EnumClass (Recommended): `DisplayMessageStatusType`. e.g. `DisplayMessageStatusType.accepted`
