...
class GenClearDisplayMessageResponse(Base_OCPP_Struct_V2_0_1):

    @staticmethod
    def generate(
        status: str | ClearMessageStatusType,
        status_info: dict | None = None,
        custom_data: dict | None = None
    ) -> call_result.ClearDisplayMessage:
        """
        Generate ClearDisplayMessageResponse

        - Args: 
            - status(str): 
                - Returns whether the Charging Station has been able to remove the message. 
                - Enum: `Accepted`, `Unknown`
                - Or use EnumClass (Recommended): `ClearMessageStatusType`. e.g. `ClearMessageStatusType.accepted`
