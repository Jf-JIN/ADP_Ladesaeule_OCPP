...
class boot_notification_request(Base_OCPP_Struct_V2_0_1): 
    ...
    @staticmethod
    def generate(charging_station: dict, reason: str | BootReasonType,
                 custom_data: dict | None = None, **kwargs) -> call.BootNotification:
        """
        ...
        return call.BootNotification(
            ...
            charging_station=charging_station or kwargs.get("charging_station", None),
            reason=reason or kwargs.get("reason", None),
            custom_data=custom_data or kwargs.get("customData", None)
            
        )

    @staticmethod
    def get_charging_station(serial_number: str, model: str, modem: dict, vendor_name: str,
                             firmware_version: str, custom_data: dict | None = None) -> dict:
        ...
        if serial_number is not None:
            temp_dict["serial_number"] = serial_number
        if modem is not None:
            temp_dict["modem"] = modem
        if firmware_version is not None:
            temp_dict["firmware_version"] = firmware_version
        return temp_dict

    @staticmethod
    def get_modem(iccid: str, imsi: str, custom_data: dict | None = None) -> dict:
