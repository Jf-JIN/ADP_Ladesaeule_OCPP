...
class meter_values_request(Base_OCPP_Struct_V2_0_1):
    ...
    @staticmethod
    def generate(evse_id: int, meter_value: list,
        ...
        return call.MeterValues(
            ...
            meter_value=meter_value or kwargs["meterValue"],
    ...
    def get_sampled_value(
        value: int | float,
        context: str | ReadingContextType = ReadingContextType.sample_periodic,
        measurand: str | MeasurandType = MeasurandType.energy_active_import_register,
        location: str | LocationType = LocationType.outlet,
        ...
        if context is not None:
            # 如果输入是None, 则不添加到字典中, 使用解释器默认的参数. 该判断只是为了避免传入None而报错, 推荐使用默认值来提高兼容性, 避免解释器不支持自动填写默认值. 其他情况类似.
    ...
    @staticmethod
    def get_unit_of_measure(unit: str = "Wh", multiplier: int = 0, custom_data: dict | None = None) -> dict:
        """
        生成 UnitOfMeasure
