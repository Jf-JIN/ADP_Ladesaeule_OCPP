...
class GenCustomerInformationResponse(Base_OCPP_Struct_V2_0_1):

    @staticmethod
    def generate(
        status: str | CustomerInformationStatusType,
        status_info: dict | None = None,
        custom_data: dict | None = None
    ) -> call_result.CustomerInformation:
        """
        Generate CustomerInformationResponse

        - Args: 
            - status(str): 
                - Indicates whether the request was accepted. 
                - Enum: `Accepted`, `Rejected`, `Invalid`
                - Or use EnumClass (Recommended): `CustomerInformationStatusType`. e.g. `CustomerInformationStatusType.accepted`
