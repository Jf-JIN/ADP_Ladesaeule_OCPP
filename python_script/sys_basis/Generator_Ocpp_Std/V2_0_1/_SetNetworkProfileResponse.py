...
class GenSetNetworkProfileResponse(Base_OCPP_Struct_V2_0_1):

    @staticmethod
    def generate(
        status: str | SetNetworkProfileStatusType,
        status_info: dict | None = None,
        custom_data: dict | None = None
    ) -> call_result.SetNetworkProfile:
        """
        Generate SetNetworkProfileResponse

        - Args: 
            - status(str): 
                - Result of operation. 
                - Enum: `Accepted`, `Rejected`, `Failed`
                - Or use EnumClass (Recommended): `SetNetworkProfileStatusType`. e.g. `SetNetworkProfileStatusType.accepted`
