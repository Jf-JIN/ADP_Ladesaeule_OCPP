...
class CP_Params(AnalogDefine):
    ...
    HOST = 'ws://130.83.148.29' if 'ocpp_opt_host' not in _dct else _dct['ocpp_opt_host']  # 优化算法器OCPP服务器地址
    # HOST = 'ws://172.18.54.149'  # 优化算法器OCPP服务器地址
    PORT = 80 if 'ocpp_opt_port' not in _dct else _dct['ocpp_opt_port']
