...
% *************************** 文献 ***************************************
% Hassan, M., & Arefi, M. (2020). "Impact of electric vehicles on the power grid: A review of recent trends and future directions."
% Renewable and Sustainable Energy Reviews, 120, 109601.

% 这篇论文详细综述了电动汽车的普及对电网的影响，特别是高峰负荷期间电网压力增加的情况。

% Impact of electric vehicles on smart grid and future predictions: a survey
% https://www.researchgate.net/publication/366380176_Impact_of_electric_vehicles_on_smart_grid_and_future_predictions_a_survey
...
% 然而增加的电车数量也带来了挑战。传统的充电模式是即插即充，功率基本保持不变。在此充电模式下， 当电动汽车持有率较低时，电网对充电需求尚可以满足。然而随着持有率的增加，电网的负荷将逐渐加重，可能会导致电力系统诸多方面的问题，例如: Congestion, Simultaneity factor, Load characteristics, Generation assets connected at low voltage level, etc \cite{innovationOutlookSmartChargingForEV}. In Germany, “dumb” charging of electric vehicles under a 10 million electric vehicles by 2035 scenario would lead to a 50\% increase in low-voltage grid and transformer costs, while optimised peak shaving using smart charging would avoid these investments (Schucht, 2017)\cite{innovationOutlookSmartChargingForEV}.
...
% 为了应对上述挑战，智能充电（Smart Charging）技术应运而生。智能充电通过总和考量电价波动、用户偏好、用户需求、电网状态等诸多因素，对充电功率进行优化，为充电桩动态设计充电时间表，实现避免电网负载峰值和低谷的出现，降低充电成本，提升电网的稳定性。
...
% 综上所述，智能充电不仅是电动交通规模化发展的关键技术之一，也是支撑未来智慧电网（Smart Grid）和可再生能源大规模接入的重要手段。针对智能充电的建模、优化与控制策略研究，具有重要的理论价值和广阔的应用前景。
...
%With the development of electric vehicles, the demand for charging has also significantly increased, and charging stations can be seen everywhere. However, some charging stations still require users to configure settings to use it manually.  

%Therefore, this ADP aims to optimize and make the charging process intelligent. Users can get a better charging experience without too many input settings.
...
% 在 OCPP 框架下，我们可以通过向充电桩发送控制消息，动态调整其输出功率，以适应不同的充电策略。根据实际需求，灵活调节充电时长和功率，为智能充电的实现提供了执行基础。
...
%For other engineers who may wish to upgrade or modify the system based on this project, it is a huge advantage. However, the \ac{OCPP} library in Python does not implement certain functionalities.

%This project aims to optimize the code to address these shortcomings. Moreover, the \ac{OCPP} is continuously updated, e.g., from version 1.6 to 2.0.1. Each update can alter the library's content; for instance, the JSON files in \ac{OCPP} 2.0.1 contain much more nested information compared to those in \ac{OCPP} 1.6. Consequently, it is essential to optimize and upgrade charging stations based on the latest \ac{OCPP} version to ensure improved functionality and compatibility. 

% section 1.3
\section{Objectives}

%At the beginning of the project, the Institute for Mechatronic Systems (IMS) provided an existing \ac{CS} equipped with two buttons: one for starting the charge and the other for stopping. The device could only charge at a fixed rate, and the charging process had to be started and stopped manually. 
% 因此该项目的目标就是改进一个由IMS所提供的CS。这个CS只有2个按钮，只能实现基本功能。

% 该 ADP 的目标是修改机电系统研究所 (IMS) 提供的现有 CS，以实现智能充电。
...
% The tasks are divided into two parts: software and hardware.

% On the software side, the focus is on enabling communication based on various protocols, such as \ac{OCPP}, WebSocket, and Modbus, ensuring that different systems can successfully exchange requests and responses to control the charging current.

% Hardware selection is based on the software functionalities.

% The hardware firstly supports the control of the logical current, as well as the connection and disconnection of charging current via a contactor. In addition, three-phase current and voltage will also be monitored.


% 该项目的主要内容为以下几点：
% 搜索并阅读现有的有关标准化充电基础设施和智能充电的文献
% 了解现有的充电站情况
% 对旧的充电站进行技术升级，让它支持OCPP通信协议
% 完成一个智能充电系统的样机的搭建
% 测试并调试智能充电站的功能
...
%The ultimate goal is to achieve peak shaving, allowing charging stations to charge vehicles with low power during peak electricity demand periods, thereby reducing the burden on the power grid and preventing overload.

% section 1.4
\section{Overview}
%主要讲论文结构

% 第二章主要介绍项目涉及的硬件，包括低压和高压组件。逻辑控制部分属于低压部分，而高压部分主要由接触器和220V电源组成。
...
% 第三章介绍了充电桩系统和优化器系统的基本架构，以及这两个系统与硬件之间的通信方式。本章介绍了OCPP Python库的使用方法及其局限性、系统如何获取OCPP消息以及如何通过生成器生成消息而无需每次都检查JSON文件、信号的实现以及异步隔离。
...
% 第四章介绍了优化器的一些基本方法、优化过程中涉及的考虑因素、充电成本、时间、成本、约束条件和目标的定义，以及主要功能服务器。
...
% 第五章主要介绍充电执行逻辑、模块设计和功能，包括充电单元模块、充电数据管理器模块、用于硬件轮询的线程管理模块、Modbus通信模块和GPIO管理器模块。
...
% 第六章主要介绍如何实现一个 \ac{GUI} 来显示和管理充电数据。

Chapter 6 mainly describes how to implement a \ac{GUI} to display and manage charging data.

% 第七章介绍了两种测试 \texttt{CS} 的方法：软件仿真和实验室测试。模拟器在实验室进行实际测试之前提供了一个虚拟平台。从功能上讲，模拟器分为两部分：一部分通过读写 JSON 文件来模拟系统与 EVSE 之间的通信；另一部分通过模拟 HTTP 服务器来模拟 Shelly 信号。测试部分涵盖了测试过程中出现的问题和结果。
...
%. Simulator offers a fictitious platform before real-world testing in the lab. Functionally, the simulator is divided into two parts: one part simulates communication between the system and the EVSE by reading and writing JSON files; the other part simulates Shelly signals by emulating an HTTP server. The testing section covers the issues that occurred during testing and the results.

% 最后一章介绍了结果评估、目标实现情况以及系统可能的改进方向。
...
%为什么
%Hardware is one of the foundations of this project. This includes identifying which electrical functions are required, which components are responsible for logical control, and explaining the function and working logic of each hardware component. 
%目的
%智能充电的实现首先需要硬件的支持，为了实现这个目的，我们的智能充电桩采用了如下硬件。
...
%Our goal is to use two parts, the first part is responsible for power supply and cut-off to the vehicles, the second part outputs the control logic signals according to the software system.
...
%这些硬件实现了搭载程序，控制功率输出，给各部件提供电源，转换通讯电压，输出功率测量等功能。
...
%The EVSE WB consumes power fewer than 1 W while letting the default current setting 13A which meets our needs \cite{EVSEdatasheet}.
...
% AF30-30-00-13是一个接触器，受EVSE控制（通过X1的rel通道）。它的额定工作电流达到50A AC,适配本项目的工作情况。\cite{AF30-30-00-13}Unlike lower-powered relays, contactors generally have special structures for arc-suppression to allow them to interrupt heavy current。通过这个接触器，EVSE只会在状态正常时接通电路，。
...
% EVSE和树莓派之间通过Modbus通讯，但由于两个系统的工作电压不同，需要在Modbus通讯线路之间通过level shifter转换电压，避免损坏树莓派。同时为了方便固定安装，我们建模打印了一个安装底座将树莓派，level shifter和relaies安装在了一起，如图所示。
...
\section{Connector}
\label{sec: connector}
%这里带过介绍下其他的connectors 然后要先介绍Typ 2 Connector
...
% On the load side, it's necessary to monitor the output. Shelly Pro 3 EM captures current, voltage and power by means of flux variation and can be connected with WiFi to transmit the data. As shown in the figure (\autoref{fig:Shelly Pro 3EM}), Shelly Pro 3 EM can be assembled on DIN rail.
...
% In this system, different hardware require totally four kinds of voltage supply. The 220V network serve as the power source from which voltage levels such as 3.3V, 5V and 12V are adapted. 

% Firstly, Phoenix STEP3-PS provides 5V 3A current for Raspberry Pi on the input side. Then, Raspberry Pi outputs also 5V voltage not only as Vcc for relay but also for level shifter on the high voltage side. Secondly, Mean Well NDR75-12 transforms 220V into 12V 6.3A current which supplys the relay. Meanwhile, Raspberry Pi outputs signals with 3.3V on GPIO14 and GPIO15, this will be converted by level shifter into 5V signals to EVSE. The electrical connection is demonstrated by circuit diagram in appendix.
...
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.3\textwidth]{img/pin-out_for_typ2.jpg} % 这里替换成你的图片文件名
%     \caption{pin-out for typ2 connector
...
%软件底层作为本项目的基石，是十分重要且必要的一部分，优秀的底层设计能使系统整体更加稳定，运行速度更加快速，拓展性更高。在本章中主要是介绍了该项目系统框架的软件底层部分中用到的设计，其在优化器与树莓派充电桩部分中共同使用，包括系统框架简介，\texttt{WebSocket}通讯端口设计，获取OCPP数据的方式，生成OCPP数据的方式，对于官方库机制的使用以适配，信号槽机制的设计，同步与异步编程的使用与隔离
...
% 得益于全双工通信，\texttt{WebSocket} 允许客户端和服务器同时发送和接收数据，而无需等待对方的响应。与 HTTP 的请求-响应模型不同，\texttt{WebSocket} 连接一旦建立，就可以随时传输数据，从而减少通信延迟。\cite{websockets} \texttt{WebSocket} 连接会一直保持打开状态，直到客户端或服务器主动关闭它。

% 不过，本项目使用上下文管理器自动管理连接，而不是手动打开和关闭连接。\texttt{WebSocket} 端口的打开与关闭过程被封装在 \_\_enter\_\_ 和 \_\_exit\_\_ 方法中，开发者只需使用 with 语句即可确保在进入上下文时自动建立连接，在退出时自动关闭连接并释放资源。(补充：在异步的例子中使用 \_\_aenter\_\_ 和 \_\_aexit\_\_ )这样设计不仅简化了 \texttt{WebSocket} 端口的管理流程，避免了忘记关闭连接或异常退出时资源泄漏的问题，还减少使用try/finally这些语法，增强了代码的可读性和可维护性。
...
% 26,04评论：可以考虑留一小段简单提一下上下文管理器自动建立和关闭连接，不用太全删，后面的扩展和单例模式那块注释掉
...
%值得一提的是，目前本项目中 \texttt{WebSocket} 端口实例直接组合在 \ac{OCPP} 消息端口类中，存在一定的耦合关系。这种设计方式虽然实现简单、便于快速开发，但从软件架构设计的角度来看，\texttt{WebSocket} 层与\ac{OCPP} 协议层紧密绑定，两者生命周期强相关，不利于后续功能的扩展与维护。

%It is worth mentioning that in this project, the \texttt{WebSocket} port instance is directly integrated in the \ac{OCPP} message port class, and create a certain coupling. This design is simple to implement and facilitates quick development. However, from a software architecture perspective, the \texttt{WebSocket} layer is closely coupled with the \ac{OCPP} protocol layer, and the life cycles of the two are strongly related, which hinders the expansion and maintenance of subsequent functions.

% 个人感觉这里的这里可以提一句：To further decouple the system, some suggestions are offered in \section{Discussion}

%为了解决这个问题，更优雅的设计方式是：单独抽象（用“分离”这个词个人感觉会好些）出\texttt{WebSocket} 端口管理类，采用单例模式实现全局唯一的管理器。管理器内部维护\texttt{WebSocket} 连接池，按\texttt{id}或连接地址、端口进行管理与复用。\ac{OCPP} 协议端口类只需通过管理器获取或注册所需的\texttt{WebSocket} 连接即可，彻底解耦\texttt{WebSocket} 层与\ac{OCPP} 协议解析层，同时提升资源复用率。

%To solve this problem, a more elegant design is to abstract the \texttt{WebSocket} port management into a  class, and use the singleton mode to create the only instance - a globally unique manager.The manager maintains the \texttt{WebSocket} connection pool internally which manages and reuses connection based on connection ID, address and port. The \ac{OCPP} protocol port class only needs to obtain or register the required \texttt{WebSocket} connection through the manager, fully decoupling the \texttt{WebSocket} layer and the \ac{OCPP} protocol parsing layer, while improving the resource reuse rate.
...
% OCPP 是由开放充电联盟 (OCA) 开发和维护的协议。在设备管理方面，充电站可以向充电管理系统 (CSMS) 发送详细的诊断数据，从而方便维护。它还利用 ISO 15118 标准进行车辆安全身份验证\cite{OCPP}。由于其良好的兼容性、Python 中现有的库以及其开源协议，所以在本项目中使用 \ac{OCPP} 。在项目启动之初，该协议的最新版本是 2.0.1 版。与 \ac{OCPP} 1.6 相比，\ac{OCPP} 2.0.1 在 JSON 文件中提供了更详细的消息定义、增强了安全功能并提高了兼容性。

% 此外，官方 \ac{OCPP} 库已在 Python 中存在\cite{ocplibrary}。虽然官方库的最新版本是 v2.0.0，但在这个项目开始之前，库的最新版本是 v1.0.0，因此本项目使用 v1.0.0。由于官方库的最新版本发生了重大变化，不再支持旧版本的枚举类。因此，本项目使用的库与最新版本不兼容。
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item %对库的注册结构进行更改，同时修改传递消息的格式。不使用装饰器的方式注册方法，而是使用信号(\autoref{sec:signal})对其连接，同第一种方法一样分离处理消息和接收消息的动作，然而需要再请求端发送时同时数据的发送时间戳，然后将执行方法加入队列，启动监听循环，当消息超时，则移出队列，并反馈/报错消息超时。与第一种方法的不同点在于，使用信号可以同时解决不能对一个动作注册多个方法的问题
...
%\begin{itemize}
   ...
   % In the code \texttt{ChargePoint.\_handle\_call(self, msg)}, when the code trying to get the method registered with \texttt{@on}, it applies directly bound using a dictionary mapping. When the first message is being processed by the registered method and a second message with the same action is received, the second message has to wait for the first one to finish processing before it can be handled. As a result, when multiple clients send the same request, the system may experience a response timeout due to its inability to process subsequent messages in time. 
...
\begin{itemize}
    ...
    %\item \texttt{signal\_response} 用于接收传递接收方发来的响应信息，信号的具体内容为包含消息动作，请求消息，响应消息，请求消息的时间戳，响应消息有效性的状态(成功收到响应，响应超时，接收错误)的一个字典。当请求发送方，等待响应消息超时时，将自动接收到一个含有响应消息有效性状态为响应超时的信号数据。
...
% 信号和槽函数最初在 Qt 框架中引入，旨在降低不同系统模块之间的耦合度。Qt 是由 Qt 公司设计的跨平台框架，它简化了 \ac{GUI} 的构建\cite{Qt}。在本项目中，我们利用了信号和槽函数机制，该机制允许组件之间互不依赖地进行交互。一个信号可以连接到多个槽函数，而一个槽函数也可以连接到多个信号，从而使系统更加灵活。

% 由于这种结构是事件驱动的，因此非常适合处理事件并执行相应的代码逻辑。与回调函数相比，信号允许动态地连接和断开槽函数，而回调函数必须事先定义。因此，在执行过程中无法修改回调函数的调用。然而，由于信号与槽函数是动态连接的，因此显式地检查信号连接可能具有挑战性，尤其是在信号连接的定义分布在不同的类或模块中时。因此，在出现错误时跟踪函数和方法的调用变得非常困难。
...
% 在\autoref{sec:ocppLib}中介绍了ocpp库的调用方式，这种装饰器方法使得执行方法必须定义在\texttt{ocpp.ChargePoint}类中，这就导致ChargePoint不仅需要分发传进来的消息，同时也要执行相应的处理逻辑。这样就要求执行实例必须定义在ChargePoint中，意味着ChargePoint类是一个顶层或者较为上层的类，然而实际上可能存在同级跨类调用或多进程/线程执行，这样将使得ChargePoint类非常大，且违反了单一职能原则。例如优化器计算模块可以定义为线程，并等待数据的传入，该线程不应与ChargePoint的生命周期绑定，而应该独立存在。
% 所以需要对ChargePoint类的功能进行拆解，使ChargePoint仅是对消息进行拆包解析，在逻辑层或更高层级的类中进行分包和执行。这样就实现了WebSocket进行收发消息，ChargePoint进行消息解包分析，更高一级的类进行消息分发，并支持跨类调用。这样则降低了系统模块间的耦合度，并且保证了单一职能原则，同时也支持了对多端口多进程的扩展开发可能性，提高了扩展能力。所以决定使用信号槽机制来进行系统内通讯。
...
% 一个进程可以拥有多个线程，线程用于在单个程序中执行多个并发（假设 CPU 只有一个核心）或并行（假设 CPU 有多个核心）操作。与线程不同，协程允许暂停和恢复执行。如表 \autoref{tab:threadandcoroutine} 所示，协程在执行不同任务时无需切换上下文，或者切换上下文的代价较低。上下文是指程序在特定时刻的执行状态，包括寄存器信息、调用堆栈、内存数据等。当 CPU 在不同任务（线程、进程或协程）之间切换时，需要保存当前任务的状态并恢复下一个任务的状态，以确保程序的连续执行\cite{coroutinetutorial}。
...
% 频繁的 I/O 操作是指程序在执行过程中频繁地读写数据，而不是持续地进行计算。在汽车充电的场景中，充电站的计算需求并不高，而是需要频繁地读取命令。从表中可以看出，协程是非阻塞的，这避免了在频繁的 I/O 操作期间 CPU 被占用但未被充分利用的情况。因此，在这种场景下，协程比线程更合适。

 % Frequent I/O operations mean that during execution, a program frequently reads or writes data rather than continuously calculating. In the case of vehicle charging, the computational demand of charging stations is not high; instead, they require frequent command reading. From the table, we can see that coroutines are non-blocking, which prevents situations where the CPU is occupied but not actively utilized during frequent I/O operations. Therefore, in such scenarios, coroutines are more suitable than threads. 

%这里如果要进一步说明thread在执行顺序上的区别 可以加入thread coroutine的时序图并且语言简单描述下 ？

% 需要注意的是，如果函数内部使用了“await”语法，则必须将其声明为“async”，因为“await”只能在协程函数中使用。调用此异步函数的上级函数也必须使用 await 来等待其结果，这意味着上级函数也必须变为异步函数。换句话说，async 和 await 总是成对出现，它们可以强制将原本不需要异步操作的函数也修改为异步函数。然而大多数传统程序是以同步编程为主，异步模式的大量引入不仅加大了代码的管理和维护难度，还会降低程序的可扩展性和可读性，导致原本清晰的同步调用链变得复杂且难以追踪。因此，将异步程序与同步程序隔离是非常必要的。 
...
% 在使用了优化器之后，我们得到了比较满意的结果。下面我将选取其中一次的优化结果来展示。该次优化使用的参数如表格所示。
...
% 我们在表格的条件下分别进行了三种模式的优化，得到了以下结果。图1展示了3种模式下的充电总量随时间的变化，可以明显看出在“最短充电时间”策略下，曲线以最快的速度到达了目标充电量，而在“最小充电花销”策略下，汽车只有在开始充电6小时后，既夜晚电价开始时，才开始充电，因此也达到了最小的花销3.6欧。图2，图3，图4分别展示了在3种不同模式下使用优化器后电网功率对比变化。总的来说，所有的加了汽车充电后的总功率都没有超过最大电网功率的限制，完美的完成了优化器的作用。
...
%充电桩作为整个系统最后的执行部分，需要实现除ocpp通讯外和evse，shelly数据通讯，信号控制等多种复杂功能，所以我们将花费大量篇幅来重点介绍。
...
% 我们设计了3种不同的充电方式
 In order to meet various application scenarios and testing needs, three charging modes have been designed.
    ...
    %     \item 手动充电计划模式:\\
    %     在该模式下，系统将根据用户手动输入的充电计划表进行充电。该模式也不进行时间对齐和校对，主要用于调试和无法连接优化器的场景。充电计划完成后自动停止充电。
...
% 三个充电模式的充电前准备和充电后清理过程是相同的，动态调整模式通过优化器生成充电计划和在充电中调整充电计划来实现智能充电。智能充电充电逻辑主要包含：充电前(\autoref{item:beforeCharging})的状态检查，充电模式的匹配，充电计划表中的\hyperref[para:timeSynchronAndSplit]{时间对齐}，充电过程中(\autoref{item:inCharging})的充电时间设置，对充电计划表的\hyperref[para:chargePlanCorrection]{周期性校验}，对充电结束的判断，以及充电结束后(\autoref{item:afterCharging})对充电单元的重置及处理
...
\begin{enumerate}[label=\Alph*.]
    ...
    % Only when there is a vehicle connected can the maximum charging current information allowed by the vehicle be obtained. The maximum charging current allowed is a required parameter in all charging modes. 
   ...
   % After the external management end sends a signal to start charging, the charging unit will start all the steps of the charging process.
    ...
    % 确认充电准备完成后，充电单元需要处理和校验充电计划表。由于存在网络延迟、时间同步误差等原因，充电计划表的执行时间可能与当前实际时间有所偏差，可能出现当前时间过早或过晚的情况。为了确保充电过程的准确执行，充电单元将首先对充电计划表进行裁剪和时间同步，具体逻辑将在后文介绍。
    ...
    % 为了确保充电过程的准确性和稳定性，系统在执行每一个充电动作前，都会重新计算本次充电动作的持续时间。由于程序执行速度及外部因素的影响，实际时间间隔可能存在波动。因此，相较于单纯依赖充电计划表的相对时间段，本项目设计采用如下公式（\autoref{eq:chargePeriodCalculation}）计算充电动作的持续时间，以保证充电动作的起止时间严格符合计划：
...
% 以典型场景为例，假设三个充电桩按照计划应该在第一个时间段以1kW、1kW和4kW的功率充电，在3分钟后进入计划的第二个时间段，各自以2kW的功率充电，系统设计的总功率上限为6kW。若由于延迟，第三个充电单元开始计划的时间晚于计划时间，但系统仍以原计划的充电时间和功率充电，导致3分钟后三个充电单元实际的功率为2kW、2kW和4kW，总功率达到8kW，明显超出上限，极有可能造成设备过载或跳闸。
...
% ************
% 整点的原因是和电价表挂钩，电价表是固定时间的，不是相对时间，所以这里校正是固定时间的整时间校正
% ************
...
%     \item 请求传递：校正请求通过GPIOManager进行传递信号。该信号载着包含充电请求和必须参数的数据，从充电单元的逻辑模块发出，并进一步通过主逻辑模块发送至优化器。系统通过发送\texttt{NotifyEVChargingNeedsRequest}消息通知优化器，要求返回经过校对后的充电计划表
...
%This request flow management not only ensures efficient communication between the charging point and the optimizer but also enhances the system's stability and reliability. With this mechanism, even when multiple charging units send concurrent requests, the system can perform correction operations in an orderly and stable manner, ensuring the accuracy and continuity of the charging schedule.
...
% Raspberry Pi 通过 Modbus 与电动汽车充电设备 (EVSE) 通信。Modbus 采用主从架构，支持多种通信方式，包括 Modbus RTU、Modbus ASCII 和 Modbus TCP\cite{modbus}。

% Modbus 采用串行通信，通过一根或多根数据线逐位传输数据。典型的波特率为 9600/19200/115200 bps。几乎所有工业设备，例如 PLC、HMI 和电动汽车充电设备 (EVSE)，都支持 Modbus。此外，使用 RS-485 接口，最多可与 247 个设备通信。如果需要与多个充电设备 (CS) 通信，Modbus 也具有可扩展性。
...
% Modbus uses serial communication, which transmits data bit-by-bit through one or multiple data cables. Typical baud rates are 9600/19200/115200 bps. Almost all industrial devices, such as PLCs, HMIs, and EVSEs, support Modbus. Additionally, using RS-485, can communicate with up to 247 devices. If in further case, there is a demand for communication with multiple CSs, it's also extendable.

% 本项目采用ModBus协议作为与EVSE模块、电能计量设备等底层设备的主要通信方式。鉴于ModBus协议的特性，其通信模型采用主从轮询机制，不支持主动数据推送或实时检测连接断开。因此，为了确保连接的有效性和通信过程中数据的可靠性，系统设计必须明确定义连接管理策略。
...
% 由于EVSE和Shelly设备本身无法主动向系统推送实时数据，因此系统需要采用轮询机制来定期获取其状态信息。然而，考虑到充电桩可能包含多个充电单元，每个充电单元都对应一组EVSE和Shelly设备，如果为每个EVSE和Shelly单独创建一个线程进行轮询，不仅会增加系统线程管理的复杂度，还会带来较高的CPU占用和内存消耗，影响系统的整体性能和响应速度。
%为了解决这一问题，本项目设计了一种高效的轮询机制，仅使用两个独立线程分别用于EVSE和Shelly的数据采集。EVSE轮询线程维护一个包含所有EVSE设备的列表，按照设定的轮询周期依次访问列表中的设备，读取相应数据并存入数据管理器。Shelly轮询线程的工作方式类似，同样维护一个Shelly设备列表，定期轮询各个设备并存储数据。ChargePoint等模块在需要访问EVSE或Shelly的数据时，无需直接与设备通信，而是通过查询数据管理器获取最新的轮询数据。这样不仅避免了多个线程并发访问设备带来的资源开销，还确保了数据的统一管理，提高了系统的稳定性，可维护性以及运行效率。
...
% \label{sec:CSWeb}
%In this project, the charging station may contain multiple charging units and support various charging modes. 
...
% \paragraph{网页实现}
% 本项目的网页界面是通过 Flask（详见 \autoref{sec:frontEndIntroduction}）建立的 Web 服务器实现的，前后端主要依赖 HTTP 进行通信。目前系统采用的是 HTTP，而非 HTTPS，因此网页的通信不包含数据保护和加密功能，相较于 HTTPS，在安全性方面存在一定缺陷。HTTP 由于不进行身份验证，用户无法确认所连接的服务器是否是合法的。此外，HTTP 采用明文传输数据，存在数据被窃听或篡改的风险。为了提升安全性，未来可以考虑引入 HTTPS 加密通信，防止数据泄露和中间人攻击。\\
...
%In terms of front-end and back-end interaction, the system uses Socket.IO for bidirectional data message transmission. The front-end web interface sends operation commands to the back-end via SockIO, such as starting or stopping charging, while the back-end pushes real-time status information to the front-end via SockIO, such as the current charging status, system logs, and charging plan execution details, enabling smooth real-time interaction.
...
%由于功能不同，服务对象不同，所以csms和cs有不同的页面功能设计
...
%     该部分用于所有数据的展示，包括EVSE的寄存器数据，Shelly测量到的数据，如电流电压功率等，以及已充电量的对比，包含预测充电量，计算充电量和shelly测量实际充电量的对比。如果充电计划表是通过优化器生成的，系统还会同步显示优化器提供的充电计划图表，以便比较实际充电情况与计划数据之间的差异。此外，为了方便数据的记录和保存，我们还提供了一个下载按钮，用于下载包含充电计划表和shelly测量数据的csv文件。
%     \item 手动输入计划表\\
...
%在进行完软件模拟之后，最重要的是进行充电测试，IMS提供一辆最高支持30A充电电流的E-Vehicle,该汽车可在连接充电线之后用遥控器控制充电开始和停止，这可作为紧急停止的方式之一。\texttt{CS}这一侧有两种充电模式需要测试，一是通过\texttt{CS}上的\texttt{Start}和\texttt{Stop}按钮以固定速率充电；其二是通过先前提到的GUI进行充电控制，在这种情况下如果需要紧急停止充电也可以通过按下\texttt{CS}上的\texttt{Stop}按钮实现。
...
% LED的问题，一开始没有指示灯显示，无法预知\texttt{CS}是否准备好充电了，于是出于可视性考虑，\texttt{Start}按钮上的LED连接一个transistor，通过树莓派的GPIO口控制LED的灯亮灭，LED若是常亮，则代表树莓派启动完成，此时可以通过GUI输入充电电流大小，当电流大小确认完成之后LED便会开始闪烁，充电开始。LED的连接线路图见附录

% Modbus的端口同一时间只能有一个线程去访问，如果要是多线程访问就会出现资源竞争的问题，比如同时对多个寄存器进行读和写，因此，我们先用线程锁强制将多线程转为串行访问，然后单例模式保证全局唯一访问点，进一步为了管理这个锁使用了上下文管理器


% 寄存器1006 在充电时一直显示bit状态是3，充电失败，而datasheet显示寄存器1006只能读取信息不能改写，但实际上dokumentation 存在错误，1006可读也可写，在通过设置寄存器1006为状态2之后，车辆能够正常充电。
...
%     \item \ac{GUI} 界面设计\\
%     本项目提供了一个 简洁易用的图形用户界面（\ac{GUI}），用于显示充电状态、日志信息以及控制操作。该界面支持基本的用户交互，如启动/停止充电、充电模式选择、提交充电计划请求、执行手动上传的充电计划表，远程管理等，方便对充电桩进行监控和控制。
%     \item 充电过程的软件模拟\\
...
% \item 并发消息处理问题\\
% 目前，系统在 OCPP 消息的并发处理方面存在一定限制。正如 \autoref{sec:ocppLib} 所述，官方提供的 OCPP Python 库默认不支持并发处理消息，因此当系统需要同时处理多个 OCPP 请求时，可能会导致阻塞或延迟，影响充电桩的实时控制能力。如果未来系统需要处理高并发 OCPP 消息交互，可以参考 \autoref{sec:ocppLib} 中提供的解决方案，对 OCPP 处理逻辑进行优化，以提升消息处理的并发程度。
...
\includepdf[pages=-]{appendix/Ladestation_v2R.pdf} 
\addcontentsline{toc}{section}{Use Case Diagram}
\includepdf[pages=-]{appendix/UseCaseDiagram.pdf} 
\addcontentsline{toc}{section}{Classes Diagram 1}
\includepdf[pages=-]{appendix/ClassesDiagram1.pdf} 
\addcontentsline{toc}{section}{Classes Diagram 2}
\includepdf[pages=-]{appendix/ClassesDiagram2.pdf} 
\addcontentsline{toc}{section}{Components Diagram}
\includepdf[pages=-]{appendix/ComponentDiagram.pdf} 
\addcontentsline{toc}{section}{Flow Diagram}
\includepdf[pages=-]{appendix/FlowDiagram.pdf}
\addcontentsline{toc}{section}{Simulation test cases}
\includepdf[pages=-]{appendix/TestCases.pdf}
